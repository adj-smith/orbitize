...
class Results(object):
    ...
    def plot_rvs(self, parallax=None, total_mass=None, object_mass=0,
        ...
        plot_orbits(): (written): Henry Ngo, Sarah Blunt, 2018
        plot_orbits(): Additions by Malena Rice, 2019
        ...
        with warnings.catch_warnings():
            ...
            objectrv = np.zeros((num_orbits_to_plot, num_epochs_to_plot))
            epochs = np.zeros((num_orbits_to_plot, num_epochs_to_plot))
            ...
            for i in np.arange(num_orbits_to_plot):
                orb_ind = choose[i]
                # Create an epochs array to plot num_epochs_to_plot points over one orbital period
                epochs[i,:] = np.linspace(start_mjd, np.min([float(start_mjd+period[orb_ind]),Time(end_year,format='decimalyear').mjd]), num_epochs_to_plot)

                # Calculate ra/dec offsets for all epochs of this orbit
                _,_, relrv0 = kepler.calc_orbit(
                    epochs[i,:], sma[orb_ind], ecc[orb_ind], inc[orb_ind], aop[orb_ind], pan[orb_ind],
                    tau[orb_ind], plx[orb_ind], mtot[orb_ind], mass=mplanet[orb_ind], tau_ref_epoch=self.tau_ref_epoch
                )

                objectrv[i,:] = relrv0+sysrv[orb_ind]
            ...
            # Plot each orbit (each segment between two points coloured using colormap)
            for i in np.arange(num_orbits_to_plot):
                points = np.array([epochs[i,:], objectrv[i,:]]).T.reshape(-1,1,2)
                segments = np.concatenate([points[:-1], points[1:]], axis=1)
                lc = LineCollection(
                    segments, cmap=cmap, norm=norm, linewidth=1.0
                )
            ...
            # # add colorbar
            if show_colorbar:
                cbar_ax = fig.add_axes([0.92, 0.15, 0.015, 0.7]) # xpos, ypos, width, height, in fraction of figure size
                cbar = mpl.colorbar.ColorbarBase(cbar_ax, cmap=cmap, norm=norm_yr, orientation='vertical', label=cbar_param)
